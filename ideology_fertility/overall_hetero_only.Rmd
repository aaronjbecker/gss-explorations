---
title: "Average Number of Children by Political Views (Heterosexual Respondents)"
author: "Your Name"
date: "`r Sys.Date()`"
...
# Restrict to heterosexual respondents only (sexornt == 3)
gss_subset <- gss_all %>%
  select(year, childs, polviews, age, sexornt, vpsu, vstrat, wtssps) %>%
  filter(
    !is.na(childs), 
    !is.na(polviews), 
    !is.na(wtssps),
    ...
    age >= min_age,
    year >= min_year,
    sexornt == 3  # Heterosexual or straight only
  ) %>%
  mutate(
...
cat("Total observations (heterosexual respondents only):", nrow(gss_subset), "\n")
cat("Year range:", min(gss_subset$year), "to", max(gss_subset$year), "\n")
cat("\nObservations by political views:\n")
...
ggplot(results_df, aes(x = year, y = childs, color = polviews_grouped, fill = polviews_grouped)) +
  ...
  labs(
    ...
    subtitle = paste0(
      "General Social Survey - Heterosexual Respondents Only (age >= ", min_age, 
      ", years >= ", min_year,
...
knitr::kable(
  ...
  caption = paste0("Recent Average (", recent_years, "-", max(results_df$year, na.rm = TRUE), ") - Heterosexual Respondents")
)
```
...
knitr::kable(
  ...
  caption = paste0(
    "Average Number of Children by Political Views (2000 onwards) - Heterosexual Respondents",
    ifelse(pool_surveys > 1, paste0(" - pooling ", pool_surveys, " surveys"), "")
  )
)
```
...
- This analysis is **restricted to heterosexual respondents only** (sexornt == 3)
